...
def generate_workingRaw_from_raw(path_raw, path_wraw):
    ...
    def get_acquisition_channel(x):
        #acq_phase = x['path'].split('\\')[-2] # acquisition phase raw/bleach
    ...
    def rawMELC_to_wrawMELC(files_pd, path_wraw):
        ...
        for k in range(len(files_pd)):
            png_to_tiff(files_pd['path'][k], path_wraw + SEPARATOR + files_pd['channel'][k] + SEPARATOR + str(int(files_pd['creation_time'][k])) + '_' + '_'.join(files_pd['fid'][k].split('_')[1:]) + '.tif')

    def rawCalib_to_wrawCalib(files_pd, path_to_save):
        files_pd = files_pd.reset_index(drop='True')
        print(path_to_save)
        for k in range(len(files_pd)):
            png_to_tiff(files_pd['path'][k], path_to_save + SEPARATOR + str(int(files_pd['creation_time'][k])) + '_' + '_'.join(files_pd['fid'][k].split('_')[1:]) + '.tif')
