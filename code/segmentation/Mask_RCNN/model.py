...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    #time_elapsed = time.time() - start_img
    #print('Time elapsed for image load: ' + str(time_elapsed))
    #start_mask = time.time()
    mask, class_ids = dataset.load_mask(image_id)
    #time_elapsed = time.time() - start_mask
    #print('Time elapsed for mask load: ' + str(time_elapsed))
    ...
    #time_elapsed = time.time() - start_rest
    #print('Time elapsed for rest load: ' + str(time_elapsed))
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    while True:
        try:
            ...
            #print('Time elapsed for data load: ' + str(time_elapsed))
            ...
            if gt_boxes.shape[0] > config.MAX_GT_INSTANCES:
                ...
                except: # exception happens due to index over the maximum length (+1)
                    print(gt_class_ids.__len__())
                    continue
...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = os.path.join(self.log_dir, "mask_rcnn_{}_epoch.h5".format(
            self.config.NAME.lower()))
